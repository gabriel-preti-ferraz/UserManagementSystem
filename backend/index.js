...
app.get("/users/:username", verifyAdmin, async (req, res) => {
    try {
        const { username } = req.params
        const searchPattern = `%${username}%`
        const result = await client.query("SELECT id, username, email, role FROM users WHERE username ILIKE $1", [searchPattern])
